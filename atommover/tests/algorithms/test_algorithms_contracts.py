...
def test_builtin_algorithms_have_required_methods(algo_cls, method_name: str) -> None:
	...
	assert callable(method), f"{algo_cls.__name__}.{method_name} is missing or not callable"
...
def test_builtin_algorithms_get_moves_return_types(algo_cls) -> None:
	...
	assert isinstance(config, (np.ndarray, AtomArray)), f"{algo_cls.__name__}.get_moves() config should be np.ndarray or AtomArray, got {type(config)}"
	assert isinstance(move_set, list), f"{algo_cls.__name__}.get_moves() move_set should be list, got {type(move_set)}"
	assert isinstance(success_flag, bool), f"{algo_cls.__name__}.get_moves() success_flag should be bool, got {type(success_flag)}"
